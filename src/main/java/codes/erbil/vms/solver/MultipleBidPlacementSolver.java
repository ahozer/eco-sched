...
@ApplicationScoped
public class MultipleBidPlacementSolver {
    ...
    public TestSolution solve(TestCase tc, List<Integer> orderedBidIndexes, String order) {
        ...
        try {
            ...
            List<List<Integer>> partitionedBidIndexes = ListUtils.partition(orderedBidIndexes, mbpBatchSize);
            int p = 1;
            for (List<Integer> partitionList : partitionedBidIndexes) {
                System.out.println("Current Partition No:" + p + "/" + partitionedBidIndexes.size());

                for (Integer bidIndex : partitionList) {
                ...
                if ((status == GRB.INF_OR_UNBD) || // EĞER SONUÇ INFEASIBLE VEYA UNBOUNDED ise UB tekrar 0 yapılır.
                    ...
                    // Partitiondaki bütün bidlerin UB 0 a eşitlenir.
                    for (Integer bidIndex : partitionList) {
    ...
    public void performFixOperationForZandY(TestCase tc, GRBVar[][] y, Map<Integer, List<GRBVar>> zByBidIndexes, Integer bidIndex) throws GRBException {
    ...
    public void performEnableOperationForZandY(TestCase tc, GRBVar[][] y, Map<Integer, List<GRBVar>> zByBidIndexes, Integer bidIndex) throws GRBException {
        for (GRBVar zByIdx : zByBidIndexes.get(bidIndex)) {
    ...
    public void performDisableOperationForZandY(TestCase tc, GRBVar[][] y, Map<Integer, List<GRBVar>> zByBidIndexes, Integer bidIndex) throws GRBException {
        for (GRBVar zByIdx : zByBidIndexes.get(bidIndex)) {
    ...
    public void setMbpBatchSize(Integer mbpBatchSize) {
        this.mbpBatchSize = mbpBatchSize;
    }
}
